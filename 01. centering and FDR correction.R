...
p_hist<-ggplot(dist, aes(x=nscore_c))+
  geom_histogram(binwidth = 0.2, position="identity", alpha=0.5, color=NA, aes(fill=factor(type, levels=c("Missense", "Nonsense", "Synonymous"))))+
...
p_hist_doubles<-ggplot(dist, aes(x=nscore_c) )+
  geom_histogram(binwidth = 0.2, position="identity", alpha=0.5, color=NA, aes(fill=factor(type, levels=c("Missense"))))+
  geom_vline(aes(xintercept=0), color="black", linetype="dashed", size=0.5)+
  theme_bw()+
