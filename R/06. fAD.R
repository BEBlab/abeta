...
onset<-left_join(onset, singles[,c("ID", "nscore_c", "sigma")], by="ID")


p_onset<-ggplot(onset, aes(Onset.mean, nscore_c))+
  geom_point(size=3)+
  geom_errorbar(aes(ymin=nscore_c-sigma, ymax=nscore_c+sigma))+
  geom_errorbar(aes(xmin=Onset.mean-Onset.sd, xmax=Onset.mean+Onset.sd))+
  theme_bw()+
  geom_text_repel(aes(label=ID ,color="#EE0011FF"), show.legend = F)+
  labs(x="Age onset" ,y="Nucleation score")+
  theme(axis.title = element_text(size=20),
        axis.text= element_text(size=16),
        panel.border = element_blank(),
        panel.grid.major = element_blank(), 
...
ggsave(p_onset, path=path, file="p_onset.pdf", width = 6, height = 5, useDingbats=FALSE)
